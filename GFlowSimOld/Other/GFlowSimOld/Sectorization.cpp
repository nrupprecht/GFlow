...
void Sectorization::initialize() {
  ...
  // Remake sectors
  if (sectors) delete [] sectors;
  sectors = new list<Particle*>[nsx*nsy]; // +1 For special sector
  // Remake particles
  if (particles) delete [] particles;
  if (positionTracker) delete [] positionTracker;
  size = plist.size();
  particles = (Particle*)aligned_alloc(128, sizeof(Particle)*size); // new Particle[size];
...
inline void Sectorization::createWallNeighborList() {
  ...
  for (int i=0; i<size; ++i) {
    ...
    for (auto &w : walls) {
      ...
      vect<> displacement = getDisplacement(p.position, w.left);
      wallDisplacement(displacement, p.sigma, w);

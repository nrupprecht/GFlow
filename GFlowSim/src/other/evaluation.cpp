...
namespace GFlowSimulation {
  ...
  EvaluationNode::EvaluationNode(char c) : val(1, c) {};

  EvaluationNode::EvaluationNode(string expr) : val(expr) {};
  ...
  void EvaluationNode::print(std::ostream& out, int level) const {
    ...
    if (left)  left->print(out, level+1);
    if (right) right->print(out, level+1);
  }
  ...
  string Eval::getNext(const string& expression, int& point) {
    ...
    while (point<expression.size()) {
      ...
      // Check
      if (c==' '); // Skip spaces.
      else if (c!='+' && c!='-' && c!='*' && c!='/' && c!='(' && c!=')') expr.push_back(c);
      else {
    ...
    for (; point<expression.size(); ++point) {
      ...
      // Low precedence operators.
        ...
        if (firstOp) {
          ...
          if (top->parent) top->parent->setRight(extra);
          else head = extra;
          // Push the node left
          extra->setLeft(top);
          // Set top

...
bool ScalarField::loadFromCSV(string filename) {
  ...
  typedef tuple<double, double, double> Trio;
  vector<Trio> values;
  double x, y, v;
  char c; // For the ','
  while (!fin.eof()) {
  ...
  for (auto t : values) {
    // Look for bounds
    if (std::get<0>(t)<minX) minX = std::get<0>(t);
    if (maxX<std::get<0>(t)) maxX = std::get<0>(t);
    if (std::get<1>(t)<minY) minY = std::get<1>(t);
    if (maxY<std::get<1>(t)) maxY = std::get<1>(t);
    // Look for resolution
    double tdx = fabs(std::get<0>(V)-std::get<0>(t)), tdy = fabs(std::get<1>(V)-std::get<1>(t));
    if (tdx<DX && 0<tdx) DX = fabs(std::get<0>(V)-std::get<0>(t));
    if (tdy<DY && 0<tdy) DY = fabs(std::get<1>(V)-std::get<1>(t));
  }
  ...
  for (const auto& v : values) {
    int x = (std::get<0>(v)-bounds.left)*idx;
    int y = (std::get<1>(v)-bounds.bottom)*idy;
    array[nsx*y+x] = std::get<2>(v);
  }
  // Return success
  return true;
}

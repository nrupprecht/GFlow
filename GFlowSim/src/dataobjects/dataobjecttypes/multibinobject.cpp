#include "multibinobject.hpp"

namespace GFlowSimulation {

  MultiBinObject::MultiBinObject(GFlow *gflow, string name, int nd) : DataObject(gflow, name, DataObjectType::MULTIBIN), ndata(max(nd, 1)) {
  ...
  void MultiBinObject::labelBins(RealType min, RealType max) {
    ...
    if (labels.size()!=nbins) labels = vector<RealType>(nbins);
    // Create labels.
    RealType dx = (max-min)/(nbins-1);
    for (int i=0; i<nbins; ++i) labels[i] = min + i*dx;
    // Set min/max.
    min_label = min;
    max_label = max;
  }

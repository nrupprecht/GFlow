...
namespace GFlowSimulation {
  ...
  bool VolumePlotObject2D::writeToFile(string fileName, bool useName) {
    ...
    if (binning.empty() || counts.empty() || data_width<=0) return true;
    ...
    for (int bx=0; bx<binX; ++bx) 
      for (int by=0; by<binY; ++by) {
        ...
        if (print_counts) fout << "," << (recorded_frames>0 ? static_cast<float>(counts[bx][by]) / recorded_frames : counts[bx][by]) << endl;
      }
    fout.close();

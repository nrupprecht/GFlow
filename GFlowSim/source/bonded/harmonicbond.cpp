...
HarmonicBond::HarmonicBond(GFlow *gflow)
    : Bond(gflow), springConstant(DEFAULT_SPRING_CONSTANT) {};

HarmonicBond::HarmonicBond(GFlow *gflow, RealType K)
    : Bond(gflow), springConstant(K) {};
...
void HarmonicBond::interact() const {
  ...
  auto x = simData->X();
  auto f = simData->F();
  auto dX = new RealType[sim_dimensions];

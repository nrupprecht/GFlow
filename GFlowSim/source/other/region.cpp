...
RectangularRegion::RectangularRegion(int d)
    : Region(d), bounds(d) {};

RectangularRegion::RectangularRegion(int d, const Bounds &b)
    : Region(d), bounds(b) {};
...
RealType &RectangularRegion::min(int d) const {
  return bounds.min[d];
}

RealType &RectangularRegion::max(int d) const {
  return bounds.max[d];
}
...
SphericalRegion::SphericalRegion(int d)
    : Region(d), _radius(0) {
  _center = new RealType[dimensions];
  zeroVec(_center, dimensions);
}

SphericalRegion::SphericalRegion(int d, const RealType *c, const RealType r)
    : Region(d), _radius(r) {
  _center = new RealType[dimensions];

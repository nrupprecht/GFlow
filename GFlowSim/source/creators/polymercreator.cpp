#include <creators/polymercreator.hpp>
// Other files
...
void PolymerCreator::createArea(HeadNode *head,
                                GFlow *gflow,
  ...
  if (dp != "") {
    if (dp == "inf") {
      imP = 0.00001; // \todo Setting this to 0 causes problems
    }
    else {
      imP = 1. / (convert<RealType>(dp) * sphere_volume(rP, sim_dimensions));
    }
  }
  if (dc != "") {
    if (dc == "inf") {
      imC = 0.00001; // \todo Setting this to 0 causes problems
    }
    else {
      imC = 1. / (convert<RealType>(dc) * sphere_volume(rC, sim_dimensions));
    }
  }
  ...
  if (pair) {
    ...
    for (int i = 0; i < number; ++i) {
      ...
      if (line) {
        ...
        group = createRandomPolymer(gflow, length, phi, idP, idC);
      }

      // Add a correlation item.
      if (i == 0) {
        gflow->addDataObject(make_shared<PersistenceLength>(gflow, group));
      }
...
inline void PolymerCreator::make_bond_objects(GFlow *gflow) {
  ...
  if (sim_dimensions == 2) {
    ...
    if (useAngle) {
      harmonicchain = make_shared<AngleHarmonicChain_3d>(gflow);
    }
  }
  else {
    if (!harmonicbonds && !useAngle) {
...
void PolymerCreator::createPolymerArrangement(vector<bool> &chain_ordering,
                                              RealType phi,
                                              RealType length) {
  // Clear out vector
  ...
  marks.push_back(0);
  for (int i = 0; i < np; ++i) {
    marks.push_back(drand48() * length * (1. - phi));
  }
  ...
  for (i = 1; i < marks.size() - 1; ++i) {
    ...
    if (drand48() < (static_cast<RealType>(ncl) - expected)) {
      ++ncl;
    }
    ...
    // Add the chain links
    for (int j = 0; j < ncl; ++j) {
      chain_ordering.push_back(false);
    }
...
Group PolymerCreator::createRandomPolymer(GFlow *gflow,
                                          RealType length,
                                          RealType phi,
                                          int idP,
                                          int idC) {
  // Valid probability, number, radii, and types are needed
  if (phi > 1 || phi < 0 || length <= 0 || rP < 0 || rC < 0 || idP < 0 || idP >= gflow->getNTypes() || idC < 0
      || idC >= gflow->getNTypes()) {
    return Group(gflow);
  }
...
Group PolymerCreator::createSinglePolymer(GFlow *gflow,
                                          const RealType *x0,
                                          const RealType *v0,
                                          const vector<bool> &chain_ordering,
                                          RealType sigma_v,
                                          int idP,
                                          int idC) {
  ...
  for (int i = 0; i < chain_ordering.size(); ++i) {
    ...
    if (correlation && useCorr) {
      correlation->add(gid2);
    }
...
Group PolymerCreator::createRandomLine(GFlow *gflow,
                                       const RealType *x,
                                       const RealType phi,
...
Group PolymerCreator::createOrderedLine(GFlow *gflow,
                                        const RealType *x,
                                        const RealType phi,
                                        const RealType length,
                                        const RealType spacing) {
...
void PolymerCreator::createParallelPolymers(GFlow *gflow,
                                            const RealType h,
                                            const RealType phi,
                                            const RealType length) {
  // Get number of dimensions

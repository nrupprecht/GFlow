#include <creators/fillareacreator.hpp>
// Other files
...
void FillAreaCreator::createArea(HeadNode *head,
                                 GFlow *gflow,
  ...
  if (parser.focus0("Velocity")) {
    ...
    else if (parser.argName() == "Zero") {
      velocityOption = 1; // Zero velocity
    }
    else {
  ...
  if (velocityOption == 0) {
    select_velocity = choose_by_normal;
  }
  else if (velocityOption == 1) {
    select_velocity = constant_velocity;
  }
  else if (velocityOption == 2) {
    select_velocity = choose_by_temperature;
  }

  // A lambda for checking if a particle falls within any excluded region.
  auto excluded_contains = [&](Vec x) {
    bool contains = false;
    for (auto r : excluded_regions) {
      contains |= r->contains(x.data);
    }
    return contains;
  };
  ...
  if (option == 0 || option == 1) {
    ...
    while (keep_adding()) {
      ...
      ParticleTemplate
          &particle_creator = particle_template_numbers.empty() ? particle_templates[0] : template_vector.at(pt);
      ...
      if (im == 0) {
        ...
        select_velocity(V.data, X.data, sigma, im, type);
      }
  ...
  if (velocityOption == 2) {
    ...
    for (int i = pf_size; i < particle_fixers.size(); ++i) {
      particle_fixers[i].velocity *= frac;
    }
  }

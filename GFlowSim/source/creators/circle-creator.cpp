#include <creators/circle-creator.hpp>
// Other files

using namespace GFlowSimulation;

void CircleCreator::createArea(HeadNode *head,
                               GFlow *gflow,
                               const std::map<string, string> &variables,
                               vector<ParticleFixer> &particle_fixers) {
  // Check if head is good
  if (head == nullptr || gflow == nullptr) {
    return;
  }
  ...
  if (center.size() != sim_dimensions) {
    throw BadDimension("Center vector for Fill: Circle needs the correct dimensionality");
  }
  // Get other data.
...
void CircleCreator::recursive_circle(const int d,
                                     const int df,
                                     const Vec &center,
                                     Vec &pos,
                                     const real R,
                                     const Bounds &process_bounds,
                                     const shared_ptr<SimData>& simData) const {
  ...
  for (real theta = 0.; theta < PI || (final_circle && theta < 2 * PI); theta += dtheta) {
    ...
    if (!final_circle) {
      recursive_circle(d + 1, df, center, pos, Rprime, process_bounds, simData);
      // Place a particle.
    }
    else {

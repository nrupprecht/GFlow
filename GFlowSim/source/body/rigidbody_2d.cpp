...
RigidBody_2d::RigidBody_2d(GFlow *gflow)
    : Body(gflow), Group(gflow) {};
...
void RigidBody_2d::post_forces() {
  ...
  if (boundaryConditions[0] == BCFlag::WRAP) {
    if (rcm[0] < bounds.min[0]) {
      rcm[0] += bnd_x;
    }
    else if (bounds.max[0] < rcm[0]) {
      rcm[0] -= bnd_x;
    }
  }
  if (boundaryConditions[1] == BCFlag::WRAP) {
    if (rcm[1] < bounds.min[1]) {
      rcm[1] += bnd_y;
    }
    else if (bounds.max[1] < rcm[1]) {
      rcm[1] -= bnd_y;
    }
  }

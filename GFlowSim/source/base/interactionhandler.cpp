...
InteractionHandler::InteractionHandler(GFlow *gflow)
    : Base(gflow),
      process_bounds(sim_dimensions),
      velocity(gflow->getSimDimensions()),
      simulation_bounds(sim_dimensions),
      border_type_up(sim_dimensions, 0),
      border_type_down(sim_dimensions, 0) {};

InteractionHandler::~InteractionHandler() {
  if (positions) {
    dealloc_array_2d(positions);
  }
  positions = nullptr;
  if (cutoff_grid) {
    dealloc_array_2d(cutoff_grid);
  }
  cutoff_grid = nullptr;
}
...
void InteractionHandler::pre_forces() {
  ...
  bool skin_depth_exceded = requires_remake ||
      (current_time - last_update > update_delay && gflow->getNumInteractions() > 0 &&
          check_needs_remake());
  ...
  if (topology->getNumProc() > 1 && update_decision_type == 0) {
    MPIObject::mpi_or(do_construct);
  }
  ...
  if (do_construct) {
    ...
    if (update_decision_type == 2) {
      if (!requires_remake) {
        last_remake_steps = update_delay_steps + 1;
      }
      update_delay_steps = last_remake_steps;
      if (topology->getNumProc() > 1) {
        MPIObject::mpi_min(update_delay_steps);
      }
    }
  ...
  else if (simData->getNeedsLocalRemake()) {
    construct_local();
  }
}
...
void InteractionHandler::construct() {
  ...
  if ((update_decision_type == 0 || update_decision_type == 2) && auto_record_positions) {
    record_positions();
  }
  ...
  if (!gflow->getInteractions().empty() && forceMaster && gflow->getUseForces()) {
    ...
    if (simData->number_ghosts() > 0) {
      traverseGhostPairs(interaction_function);
    }
...
void InteractionHandler::removeOverlapping(RealType factor) {
  ...
  traversePairs([&](int id1, int id2, int, RealType r1, RealType r2, RealType rsqr) {
    ...
    if (overlap / min(r1, r2) > factor && simData->Im(id1) > 0) {
...
void InteractionHandler::checkBounds() {
  if (topology) {
    ...
    if (process_bounds != topology->getProcessBounds()) {
      initialize();
    }
  }
}
...
void InteractionHandler::calculate_max_small_sigma() {
  ...
  if (!forceMaster || !simData || simData->size_owned() == 0) {
    return;
  }
...
bool InteractionHandler::check_needs_remake() {
  ...
  if (last_check - last_update > max_update_delay) {
    needs_remake = true;
  }
  ...
  update_delay = min(max_update_delay,
                     mv_ratio_tolerance * motion_factor * (last_check - last_update) / motion_ratio);
...
void InteractionHandler::record_positions() {
  ...
  for (int i = 0; i < samples; ++i) {
    copyVec(simData->X(start - 1 - i), positions[i], sim_dimensions);
  }
}
...
RealType InteractionHandler::find_max_motion() {
  ...
  for (int i = 0; i < size; ++i) {
    ...
    if (dsqr < max_plausible && dsqr > maxDSqr) {
      maxDSqr = dsqr;
    }
  }
...
inline void InteractionHandler::set_up_grids() {
  ...
  if (gflow->getUseForces()) {
    ...
    cutoff_grid = 0 < ntypes ? alloc_array_2d<RealType>(ntypes, ntypes) : nullptr;
  ...
  for (int d = 0; d < sim_dimensions; ++d) {
    if (bcs[d] == BCFlag::WRAP) {
      ...
      // There are one or more processors above and below this one.
        ...
        if (process_bounds.max[d] == simulation_bounds.max[d]) {
          border_type_up[d] = 2;
        }
        ...
        // If this processor expects ghost particles that are wrapped.
        if (process_bounds.min[d] == simulation_bounds.min[d]) {
          border_type_down[d] = 2;
        }
          // Ghost particles, but not wrapped ones.
        else {
...
void InteractionHandler::pair_interaction(const int id1, const int id2, const int list) {
  ...
  if (pair_force) {
    pair_force->addPair(id1, id2, list);
  }
}

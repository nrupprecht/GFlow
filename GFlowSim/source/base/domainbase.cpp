...
DomainBase::DomainBase(GFlow *gflow)
    : InteractionHandler(gflow), dims(sim_dimensions, 0), widths(sim_dimensions, 0.),
      inverseW(sim_dimensions, 0.),
      dim_shift_up(sim_dimensions, 0), dim_shift_down(sim_dimensions, 0),
      products(sim_dimensions + 1, 0) {};
...
void DomainBase::initialize() {
  ...
  if (process_bounds.vol() <= 0 || std::isnan(process_bounds.vol()) || simData == nullptr ||
      simData->size_owned() == 0) {
    return;
  }
...
int DomainBase::get_cell_index(const RealType *x) {
  ...
  for (int d = 0; d < sim_dimensions; ++d) {
    ...
    if (dims[d] - dim_shift_up[d] <= index) {
      index = dims[d] - dim_shift_up[d] - 1;
    }
    else if (index < dim_shift_down[d]) {
      index = dim_shift_down[d];
    }
...
int DomainBase::get_halo_cell_index(const RealType *x) {
  ...
  for (int d = 0; d < sim_dimensions; ++d) {
    ...
    if (x[d] + epsilon >= process_bounds.max[d]) {
      index = dims[d] - 1;
    }
    else if (x[d] - epsilon <= process_bounds.min[d]) {
      index = 0;
      // Otherwise, find cell index as usual.
    }
    else {
      index = max(static_cast<int>((x[d] - process_bounds.min[d]) * inverseW[d] + dim_shift_down[d]), 0);
    }
    linear += index * products[d + 1];
  }
  // Return the index

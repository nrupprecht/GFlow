...
DomainList::DomainList(GFlow *gflow)
    : DomainBase(gflow) {};
...
void DomainList::traversePairs(PairFunction body) {
  ...
  if (sim_dimensions != dimensions) {
    throw BadDimension("Dimensions in domain-list is currently hard coded.");
  }
...
void DomainList::structure_updates() {
  // Clear particle list
  if (particle_list.size() < simData->size_owned()) {
    particle_list = vector<int>(simData->size_owned(), -1);
  }
  else {

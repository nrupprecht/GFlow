...
TemperatureModifier::TemperatureModifier(GFlow *gflow, RealType t)
    : Modifier(gflow),
      temperature(t), viscosity(DEFAULT_VISCOSITY), lastUpdate(0), updateDelay(0.01) {};

TemperatureModifier::TemperatureModifier(GFlow *gflow, RealType t, RealType h)
    : Modifier(gflow),
...
void TemperatureModifier::post_forces() {
  ...
  if (time - lastUpdate < updateDelay || temperature <= 0) {
    return;
  }

  if (sim_dimensions > 4) {
    throw BadDimension("TemperatureModifier assumes the dimensionality is not greater than four.");
  }

...
ConstantVelocityDistance::ConstantVelocityDistance(GFlow *gflow, int g_id, RealType *v, RealType d)
    : Modifier(gflow), global_id(g_id), distance(d) {
...
void ConstantVelocityDistance::post_forces() {
  ...
  if (!moving) {
    ...
    if (sqr(displacement, sim_dimensions) > sqr(distance)) {
      moving = false;
    }
  }
}

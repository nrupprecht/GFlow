...
EndingSnapshot::EndingSnapshot(GFlow *gflow)
    : DataObject(gflow, "Snapshot") {
  // The data to gather
  vector_data_entries.emplace_back("X");
  vector_data_entries.emplace_back("V");
  scalar_data_entries.emplace_back("Sg");
  scalar_data_entries.emplace_back("StripeX");
  integer_data_entries.emplace_back("Type");
...
bool EndingSnapshot::writeToFile(string fileName, bool useName) {
  ...
  try {
    ...
    if (sim_dimensions < 3) {
      ...
      vis.createImage3d(dirName + "/kinetic.bmp", final_data);
    }
    // Direction snapshot
    ...
    if (sim_dimensions < 3) {
      ...
      vis.createImage3d(dirName + "/direction.bmp", final_data);
    }
    // Type snapshot
    if (gflow->getNTypes() > 1) {
      vis.setColorOption(0);
      if (sim_dimensions < 3) {
        vis.createImage(dirName + "/types.bmp", final_data);
      }
      else {
        vis.createImage3d(dirName + "/types.bmp", final_data);
      }
    }
    ...
    if (simData->getScalarData("StripeX") != -1) {
      ...
      if (sim_dimensions < 3) {
        vis.createImage(dirName + "/stripe-x.bmp", final_data);
      }
      else {
        vis.createImage3d(dirName + "/stripe-x.bmp", final_data);
      }
    }
  }
  catch (...) {

...
TwoGroupStatistics::TwoGroupStatistics(GFlow *gflow)
    : DataObject(gflow, "TwoGroupStatistics"), group1(gflow), group2(gflow) {
  setBins(50, 101);
};
...
void TwoGroupStatistics::post_step() {
  ...
  if (!DataObject::_check() || group1.empty() || group2.empty()) {
    return;
  }
...
bool TwoGroupStatistics::writeToFile(string fileName, bool useName) {
  ...
  if (count_v.empty() && count_f.empty()) {
    return true;
  }

...
BondedEnergyData::BondedEnergyData(GFlow *gflow, bool ave)
    : GraphObject(gflow, "BondEnergy", "time", "bond energy"), useAve(ave) {};
...
void BondedEnergyData::post_step() {
  ...
  for (auto it : bonded_interactions) {
    energy += it->getPotential();
  }

  // Store data. These functions work correctly with multiprocessor runs.
  if (useAve) {
    gatherAverageData(gflow->getElapsedTime(), energy, simData->size_owned());
  }
  else {

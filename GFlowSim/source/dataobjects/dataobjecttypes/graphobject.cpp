...
GraphObject::GraphObject(GFlow* gflow, const std::string &name)
    : DataObject(gflow, name, DataObjectType::GRAPH) {};

GraphObject::GraphObject(GFlow* gflow, const std::string &name, std::string ax, std::string ay)
    : DataObject(gflow, name, DataObjectType::GRAPH), axis_x(std::move(ax)), axis_y(std::move(ay)) {};
  ...
  if (nbins > 0) {
    ...
    if (nbins > 1) {
      ...
      for (int i = 0; i < nbins; ++i) {
        data[i].first = min + dbin * i;
      }
    }
    else {
      data[0].first = 0.5 * (min + max);
    }
  }
}

bool GraphObject::writeToFile(std::string fileName, bool useName) {

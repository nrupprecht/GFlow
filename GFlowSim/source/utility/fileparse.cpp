...
FileParse::FileParse()
    : level(0), currentHead(nullptr) {};

HeadNode *FileParse::parseFile(const string& configFile) {
  // Create a parent head
  auto root = new HeadNode;
  currentHead = root;
  // Create filestream
...
inline void FileParse::checkComment(std::ifstream &fin) {
  ...
  if (c == '/') {
    passComment(fin, false);
  }
  else if (c == '*') {
    passComment(fin, true);
  }
  else {
    throw UnexpectedToken("In checkComment: Token: [" + toStr(c) + "].");
  }
}

inline std::string FileParse::tabs() {
  string tbs = "";

...
namespace GFlowSimulation {
  ...
  **/
  ...
  public:
    ...
    bool load_and_create(const string&, const string&);
    ...
    void createVideo2d(const string&, const vector<vector<float> >&);

    //! \brief Create a directory filled with BMP renderings of the system, for 3D data.
    void createVideo3d(const string&, const vector<vector<float> >&);
  ...
  private:
    ...
    inline bool do_checks(const vector<float>&) const;
    ...
    inline void get_values(const float*, const float*&, float&, const float*&, float&, int&) const;

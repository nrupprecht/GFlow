...
template<int dims, class ForceType, bool using_history>
class VerletList : public Interaction {
 public:
  //! \brief Constructor.
  explicit VerletList(GFlow *gflow)
      : Interaction(gflow) {
  ...
  void addPair(const int id1, const int id2, const int list) override {
  ...
  //! \brief Iterate through all
  void interact_ghosts() const override {
    // Common tasks
 ...
 private:
  ...
  template<bool wrapping, int first_particle_type = 0>
  inline void dolist(const int list) const {
    // Reference to the requested list.
    const vector<int> &verlet_list = verlet[list];
    // If there are no particles, return.
    if (verlet_list.empty()) {
      return;
    }
    ...
    for (int i = 0; i < heads.size(); ++i) {
      ...
      for (int j = first[i]; j < first[i + 1]; ++j) {
        ...
        if (wrapping) {
          harmonic_correction<dims>(bcs, X, widths);
        }
...
template<int dims, class ForceType, bool using_history>
class VerletListVecPairs : public Interaction {
 public:
  //! \brief Constructor.
  explicit VerletListVecPairs(GFlow *gflow)
      : Interaction(gflow) {
 ...
 private:
  ...
  //! \brief Do the forces for a single list.
  template<bool wrapping, int first_particle_type = 0>
  inline void dolist(const vector<int> &verlet_list) const {
...
template<int dims, class ForceType, bool using_history>
class VerletListPairs : public Interaction {
 public:
  //! \brief Constructor.
  explicit VerletListPairs(GFlow *gflow)
      : Interaction(gflow) {
 ...
 private:
  ...
  //! \brief Do the forces for a single list.
  template<bool wrapping, int first_particle_type = 0>
  inline void dolist(const vector<pair<int, int> > &verlet_list) const {
    // If the list is empty, return.
    if (verlet_list.empty()) {
      return;
    }
    ...
    for (const auto &[id1, id2] : verlet_list) {
      // Calculate displacement.
      subtract_vec<dims>(x0(id1), x1(id2), X);
      // Harmonic corrections to distance.
      if constexpr (wrapping) {
        harmonic_correction<dims>(bcs, X, widths);
      }
      // Calculate squared distance
      rsqr = dot_vec<dims>(X, X);
  ...
  std::vector<std::pair<int, int> > verlet_pairs[3];
};

template<int dims, class ForceType, bool using_history>
class VerletListVector : public Interaction {
 public:
  //! \brief Constructor.
  VerletListVector(GFlow *gflow)
      : Interaction(gflow) {};
 ...
 private:
  ...
  //! \brief Do the forces for a single list.
  template<bool wrapping, int first_particle_type>
  inline void dolist(const std::vector<std::vector<int>> &neighborlist) const {
    // If the list is empty, return.
  ...
  std::vector<std::vector<int>> neighbor_list[3];
